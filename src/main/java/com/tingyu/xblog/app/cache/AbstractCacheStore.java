package com.tingyu.xblog.app.cache;
...
public abstract class AbstractCacheStore<K, V> implements CacheStore<K, V> {
    ...
    public Optional<V> get(K key) {
        ...
        return getInternal(key).map(cacheWrapper -> {
            // Check expiration
            if (cacheWrapper.getExpireAt() != null && cacheWrapper.getExpireAt().before(com.tingyu.xblog.app.utils.DateUtils.now())) {
    ...
    private CacheWrapper<V> buildCacheWrapper(@NonNull V value, long timeout, @Nullable TimeUnit timeUnit) {
        ...
        Date now = com.tingyu.xblog.app.utils.DateUtils.now();
